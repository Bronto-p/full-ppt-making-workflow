...
def main():
    ...
    with locked_jobs(run_dir) as jobs:
        ...
        if slots <= 0:
            raise SystemExit(
                "No page dispatch slots available. "
                f"max_concurrent_pages={jobs.get('max_concurrent_pages')}, active dispatches already at limit."
            )
