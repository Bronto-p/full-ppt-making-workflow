...
def main():
    ...
    with locked_jobs(run_dir) as jobs:
        ...
        page["status"] = "recorded" if validation_passed else "validation_failed"
        update_jobs_run_status(jobs)
        run_status = jobs.get("run_status")
        page_id = page["page_id"]
        status = page["status"]
    if run_status == "pages_recorded":
        set_run_status(run_dir, "pages_recorded", "all pages recorded")
    elif run_status == "validation_failed":
        set_run_status(run_dir, "validation_failed", f"{page_id} validation failed")
    print(json.dumps({"page_id": page_id, "status": status, "validation_passed": validation_passed}, ensure_ascii=False))
