...
def _build_prompt(
    *,
    ...
    if global_style_references:
        count = len(global_style_references)
        image_label = "Image 1" if count == 1 else f"Images 1-{count}"
        prompt_parts.append(
            "## Style Reference Rule\n"
            f"Use {image_label} as approved sample-slide style references. Match their palette, "
            "typography mood, density, texture, and overall visual identity. Do not copy "
            "their exact layout unless this slide's layout explicitly asks for it.\n"
        )
...
def main() -> int:
    ...
    raw_style_references: List[Any] = []
    approved_style_references = spec.get("approved_style_references")
    if approved_style_references is not None:
        if not isinstance(approved_style_references, list):
            _die("approved_style_references must be an array when present.")
        raw_style_references.extend(approved_style_references)
    approved_style_reference = spec.get("approved_style_reference")
    if approved_style_reference is not None:
        if not isinstance(approved_style_reference, dict):
            _die("approved_style_reference must be an object when present.")
        raw_style_references.insert(0, approved_style_reference)

    global_style_references: List[Dict[str, Any]] = []
    for index, reference in enumerate(raw_style_references, start=1):
        if not isinstance(reference, dict):
            _die(f"approved style reference {index} must be an object.")
        normalized = dict(reference)
        if isinstance(normalized.get("path"), str):
            normalized["path"] = _resolve_image_path(normalized["path"], base_dir=spec_dir)
        normalized.setdefault("role", f"approved sample slide style reference {index}")
        normalized.setdefault("fidelity", "match style only; do not copy layout or content")
        global_style_references.append(normalized)
