...
- For any sample or final slide with strict source images (client photos, certificates, screenshots, charts, proof documents, papers, logos, or other content assets), the selected backend must be able to actually receive or see those images. A text prompt containing local file paths is not enough.
- If the built-in image tool is available but its callable interface cannot attach or expose the required local images to the image model for this slide, do not use built-in mode for that slide. Select CLI/API fallback if it can pass the images, or record a blocker if no backend can satisfy the image-input requirement.
...
- Before generating the first image, tell the user which tool availability you checked, which backend you plan to use, why fallback is or is not needed, and ask for confirmation. If the user has already explicitly authorized generating the sample after seeing the plan, this backend notice can be part of the work update and does not require a second stop unless changing backend affects asset fidelity, cost, credentials, or output format. Do not treat being in a specific agent environment as proof that the built-in image tool is available.
